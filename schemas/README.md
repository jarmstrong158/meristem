...
| `items.schema.json` | items | items with `slot` + `stats` (worn slots equip on pickup and move the player's atk/def), a `sprite` descriptor, rarity_tiers, drop_tables (→ entities.enemies, items) |
